...
function setExplorerFixBtns(enabled) {
  ['exp-fix-names-btn','exp-fix-dupes-btn','exp-fix-missing-btn','exp-export-dat-btn'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.disabled = !enabled;
  });
  const res = document.getElementById('exp-fix-result');
  if (res) res.textContent = '';
}

function explorerFixNames() {
  if (!explorerGames.length) return;
  const illegal = /[<>:"/\\|?*\x00-\x1F]/g;
  let fixed = 0;
  explorerGames = explorerGames.map(g => {
    const clean = (g.name||'').replace(illegal, '_').trim();
    if (clean !== g.name) { fixed++; return { ...g, name: clean }; }
    return g;
  });
  window.explorerGamesForSplitter = explorerGames;
  applySortAndRender();
  showFixResult(`✓ ${fixed} name${fixed!==1?'s':''} sanitized`);
}

function explorerRemoveDupes() {
  if (!explorerGames.length) return;
  const seen = new Set();
  const before = explorerGames.length;
  explorerGames = explorerGames.filter(g => {
    const roms = (g.rom||[]);
    const fp = roms.length
      ? roms.map(r=>`${(r.crc||'').toLowerCase()}|${(r.sha1||'').toLowerCase()}`).sort().join('::')
      : `name:${(g.name||'').toLowerCase()}`;
    if (seen.has(fp)) return false;
    seen.add(fp); return true;
  });
  window.explorerGamesForSplitter = explorerGames;
  const removed = before - explorerGames.length;
  applySortAndRender();
  showFixResult(`✓ ${removed} duplicate${removed!==1?'s':''} removed`);
}

function explorerRemoveMissingHashes() {
  if (!explorerGames.length) return;
  const before = explorerGames.length;
  explorerGames = explorerGames.filter(g => {
    const roms = g.rom || [];
    if (!roms.length) return false;
    return roms.some(r => r.crc || r.sha1 || r.md5);
  });
  window.explorerGamesForSplitter = explorerGames;
  const removed = before - explorerGames.length;
  applySortAndRender();
  showFixResult(`✓ ${removed} entr${removed!==1?'ies':'y'} without hashes removed`);
}

async function explorerExportDat() {
  if (!explorerGames.length) return;
  const outputDir = await window.electronAPI.selectOutputFolder?.();
  if (!outputDir) return;

  const header = window.explorerHeaderForSplitter || {};
  const date   = new Date().toISOString().slice(0,10);
  const name   = header.name || 'Exported DAT';
  ...
  const lines = [
    ...
    '  <header>',
    `    <name>${esc2(name)}</name>`,
    `    <description>${esc2(header.description || name)}</description>`,
    `    <version>${esc2(header.version || date)}</version>`,
    `    <date>${date}</date>`,
    `    <author>${esc2(header.author || 'DAT//ROMMANAGER')}</author>`,
    '  </header>',
  ];

  for (const g of explorerGames) {
    lines.push(`  <game name="${esc2(g.name||'')}">`,
      `    <description>${esc2(g.description||g.name||'')}</description>`);
    for (const r of (g.rom||[])) {
      const attrs = ['name','size','crc','md5','sha1','sha256']
        .filter(k=>r[k]).map(k=>`${k}="${esc2(String(r[k]))}"`).join(' ');
      if (attrs) lines.push(`    <rom ${attrs}/>`);
    }
    lines.push('  </game>');
  }
  lines.push('</datafile>');
  ...
  const result = await window.electronAPI.saveTextDat?.({
    outputDir,
    filename: `${safeName}-fixed.dat`,
    content:  lines.join('\n'),
  });

  if (result?.ok) {
    showFixResult(`✓ Exported ${explorerGames.length.toLocaleString()} entries`);
  } else {
    showFixResult(`✗ Export failed: ${result?.error || 'unknown error'}`);
  }
}

function showFixResult(msg) {
  const el = document.getElementById('exp-fix-result');
...
// ── PUBLIC: load a file from another module (e.g. Splitter) ──
async function loadDatFileInExplorer(filePath) {
  if (!filePath) return;
  await loadExplorerFile(filePath);
}
