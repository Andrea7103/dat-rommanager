...
function gameFingerprint(g) {
  const roms = g.rom || [];
  if (!roms.length) {
    // Games with no dumped ROMs (MAME "device"/slot placeholders, BIOS
    // stubs, etc.) have nothing to hash — fall back to the name so they
    // don't all collide under the same empty-string key and overwrite
    // each other in the fingerprint map.
    return `noromfp::${(g.name||'').toLowerCase()}`;
  }
...
async function saveDiffOutput() {
  ...
  for (const grp of groups) {
    ...
    const writeResult = await window.electronAPI.saveTextDat({
      outputDir: diffOutputFolder,
      filename,
      content,
    });
    ...
    if (!writeResult.ok) {
      hideDiffProgress();
      document.getElementById('diff-save-btn').disabled = false;
      alert(`Failed to write ${filename}: ${writeResult.error || 'unknown error'}`);
      return;
    }

    saved.push({ label: grp.label, count: grp.games.length, path: writeResult.path });
    step += stepSize;
  }
