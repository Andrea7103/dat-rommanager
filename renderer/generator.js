...
function initGenDropZone() {
  ...
  zone.addEventListener('drop', async e => {
    ...
    // Sanity-check what was dropped against the active mode, so a
    // misdrop fails with a clear message instead of a confusing
    // downstream error (e.g. spawning a folder as if it were an EXE).
    const entry = e.dataTransfer.items?.[0]?.webkitGetAsEntry?.();
    const isDirectory = entry ? entry.isDirectory : false;

    if (genMode === 'folder' && !isDirectory) {
      genLog('error', `❌ Expected a folder, got a file: ${item.name}. Switch to EMULATOR EXE mode to load an executable instead.`);
      return;
    }
    if (genMode === 'emulator' && isDirectory) {
      genLog('error', `❌ Expected an executable file, got a folder: ${item.name}. Switch to ROM FOLDER mode to scan a folder instead.`);
      return;
    }
...
async function handleGenDrop(filePath, fileName) {
  if (genMode === 'folder') {
    ...
    // Accept EXE
    const isNewExe = filePath !== genExePath;
    genExePath = filePath;
    if (isNewExe) {
      // A different executable was loaded — let its profile autofill
      // the args field again, instead of staying stuck on the old value.
      const argsEl = document.getElementById('gen-emu-args');
      delete argsEl.dataset.userEdited;
    }
...
function resetGenDrop() {
  ...
  const argsEl = document.getElementById('gen-emu-args');
  if (argsEl) { argsEl.value = ''; delete argsEl.dataset.userEdited; }
}
