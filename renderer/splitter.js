...
function renderPreview() {
  ...
  document.getElementById('spl-preview-header').innerHTML =
    ...
    `<span style="color:var(--green-dim);font-size:9px;letter-spacing:1px;margin-left:8px">${modeLabels}</span>` +
    `<span style="margin-left:auto;color:var(--text-dim)">${totalEnabled} selected</span>`;
  ...
  container.innerHTML = splitterPreview.map((g, i) => `
    <div class="spl-preview-row ${g.enabled?'':'disabled'}" id="splrow-${i}">
      <div class="spl-preview-check" onclick="toggleSplitGroup(${i})">
  ...
  const html = `
    <div class="spl-summary-box">
      ...
      <div class="spl-summary-rows">
        ${results.map(r=>`<div class="spl-sum-row">
