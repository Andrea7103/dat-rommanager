...
ipcMain.handle('scan-rom-folder', async (event, { folderPath, recursive }) => {
  ...
  // Files that are OS/filesystem noise, never real ROM content
  const JUNK_FILES = new Set(['thumbs.db', 'desktop.ini', '.ds_store', '.directory']);
  ...
  // Walk directory (depth-capped as a guard against symlink cycles)
  const allFiles = [];
  const MAX_DEPTH = 32;
  async function walk(dir, depth) {
    if (depth > MAX_DEPTH) return;
  ...
  const games = [];
  const usedNames = new Map(); // baseName → count, so duplicate filenames across subfolders don't collide
  let done = 0;
  let skipped = 0;
  ...
  for (const f of allFiles) {
    ...
    let stat;
    try { stat = await fsPromises.stat(f.full); } catch {
      done++; skipped++;
      send('log', { msg: `⚠ Skipped (cannot stat): ${f.rel}`, cls: 'gen-warn' });
      continue;
    }
    ...
    await new Promise((resolve) => {
      ...
      stream.on('error', err => { readError = err; resolve(); });
    });

    if (readError) {
      done++; skipped++;
      send('log', { msg: `❌ Skipped (read error): ${f.rel} — ${readError.message}`, cls: 'gen-error' });
      continue;
    }
    ...
    // Ensure unique game name even if the same filename appears in
    // multiple subfolders (recursive scan) — DAT game names must be unique.
    let baseName = path.basename(f.name, path.extname(f.name));
    const seenCount = usedNames.get(baseName) || 0;
    usedNames.set(baseName, seenCount + 1);
    const gameName = seenCount === 0 ? baseName : `${baseName} (${seenCount + 1})`;
  ...
  const summary = skipped > 0
    ? `✅ Done — ${games.length} games generated (${skipped} file${skipped!==1?'s':''} skipped)`
    : `✅ Done — ${games.length} games generated`;
  send('log', { msg: summary, cls: 'gen-success' });
...
ipcMain.handle('run-emulator', async (event, { exePath, args }) => {
  ...
  // Tokenize args respecting "quoted paths with spaces"
  const tokenizeArgs = (str) => {
    const out = [];
    const re = /"([^"]*)"|'([^']*)'|(\S+)/g;
    let m;
  ...
  return new Promise((resolve) => {
    ...
    const child = spawn(exePath, tokenizeArgs(args), {
      cwd: workDir,
      windowsHide: true,
    });
    ...
    child.on('close', code => {
      // Exit code 0 = success. Anything else is a real failure — don't
      // silently hand the renderer garbage/empty XML to parse.
      ...
      if (success) {
        ...
        const tail = stderr.trim().slice(-300) || '(no error output)';
        send('log', { msg: `❌ Process exited with error code ${code} — aborting`, cls: 'gen-error' });
        resolve({ ok: false, error: `Emulator exited with code ${code}: ${tail}` });
      }
    });
  });
});
...
// ============================================================
// IPC: Save a single pre-built DAT/text file (path.join = cross-platform)
// Used by Diff / Explorer Fix&Export / Checksum export — they build
// the XML content themselves and just need it written to disk.
// ============================================================
ipcMain.handle('save-text-dat', async (event, { outputDir, filename, content }) => {
  try {
